...
`ami_eks_al2023` takes seven optional overrides: `EKS_VERSION`, `INSTANCE_TYPE`,
`NVIDIA_DRIVER_VERSION`, `NVIDIA_KERNEL_MODULES` with the value `proprietary` or `open`,
`NVIDIA_CONTAINER_TOOLKIT_VERSION`, `NVIDIA_VERIFY_GPU`, and `PCLUSTER_AMI_REGION`.

By default the build host has to be an instance type whose GPU the chosen driver supports, because
the playbook runs `nvidia-smi` on the build instance and asserts that it reports the pinned version.
Set `NVIDIA_VERIFY_GPU=false` to build on a host without a GPU; the playbook then asserts that the
package is installed at the pinned version instead. Neither check covers the produced AMI on another
instance type: what proves an image drives the GPUs of the type it will run on is the node-side check
in [`architectures/amazon-eks`](../architectures/amazon-eks).

`NVIDIA_KERNEL_MODULES=open` selects the open kernel modules. Which flavour enumerates a given GPU
depends on the driver and the GPU together, so the flavour is an input rather than a constant; see
[NVIDIA's open GPU kernel modules](https://github.com/NVIDIA/open-gpu-kernel-modules#compatible-gpus)
for which GPUs they cover.
